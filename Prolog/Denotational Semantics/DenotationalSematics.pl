%parse tree generators for the input list of tokens.
p(fp(K)) --> 	k(K),[.].
...
%predicates to test parse tree generation in all 8 cases.
...
%Store related semantic functions.
initialize_store([]).
access(Id,[],0).
...
%semantics for evaluation of the input.
...
%test predicates for evaluating all 8 cases.								
